...
: Node("velocity_controller", node_options)
{
  ...
  pid_ks_ = declare_parameter("pid_ks", 0.5);
  ...
  const ControlData & control_data)
{
  ...
  if (current_control_state == ControlState::DRIVE) {
    ...
    // RCLCPP_INFO(this->get_logger(), "raw_ctrl_cmd.vel + raw_ctrl_cmd.acc: %f", raw_ctrl_cmd.vel + raw_ctrl_cmd.acc);
    double feedback_vel = pidresult(target_motion.vel, raw_ctrl_cmd.acc, pid_ks_);
    raw_ctrl_cmd.vel = feedback_vel;
    raw_ctrl_cmd.acc = 0;
    // RCLCPP_INFO(this->get_logger(), "raw_ctrl_cmd.vel: %f", raw_ctrl_cmd.vel);
    RCLCPP_DEBUG(
      get_logger(),
  ...
  const Motion target_motion, const double dt, const double current_vel)
{
  ...
  // RCLCPP_INFO(this->get_logger(), "current_vel_abs: %f", current_vel_abs);
  // RCLCPP_INFO(this->get_logger(), "target_vel_abs: %f", target_vel_abs);
  // RCLCPP_INFO(this->get_logger(), "target_vel_abs - current_vel_abs: %f", target_vel_abs - current_vel_abs);
  // RCLCPP_INFO(this->get_logger(), "error_vel_filtered: %f", error_vel_filtered);
  ...
  // RCLCPP_INFO(this->get_logger(), "target_motion.acc: %f", target_motion.acc);
  // RCLCPP_INFO(this->get_logger(), "pid_acc: %f", pid_acc);
  // RCLCPP_INFO(this->get_logger(), "feedback_acc: %f", feedback_acc);
...
double VelocityController::pidresult(
  const double targetspeed, const double feedbackspeed, const double Ks_)
{
  double result_ = targetspeed + feedbackspeed;
  if((feedbackspeed * targetspeed) < 0) {
    result_= fabs(feedbackspeed) > (fabs(targetspeed) * Ks_) ? 0 : result_;
  }
  result_ = (result_ * targetspeed) > 0 ? result_ : 0;
  return result_;
}
