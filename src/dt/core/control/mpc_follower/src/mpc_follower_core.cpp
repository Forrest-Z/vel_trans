...
: Node("mpc_follower", node_options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
{
  ...
  sub_all_path_ = create_subscription<autoware_planning_msgs::msg::Trajectory>(
    "/freespace_planner/output/all_trajectory", rclcpp::QoS{1},
    std::bind(&MPCFollower::onAllTrajectory, this, _1));
  ...
  pub_debug_all_marker_ = create_publisher<visualization_msgs::msg::MarkerArray>("~/debug/all_markers", 10);
...
void MPCFollower::onAllTrajectory(const autoware_planning_msgs::msg::Trajectory::SharedPtr msg)
{
  if (!current_pose_ptr_) {
    return;
  }

  all_trajectory_ptr_ = msg;
  ...
  /* resampling */
  MPCUtils::convertToMPCTrajectory(*all_trajectory_ptr_, &mpc_traj_raw);
  ...
  /* publish debug marker */
  {
    ...
    std::string frame = msg->header.frame_id;
    m = convertTrajToMarker(mpc_traj_raw, "trajectory raw", 0.9, 0.5, 0.0, 0.05, frame, stamp);
    pub_debug_all_marker_->publish(m);
