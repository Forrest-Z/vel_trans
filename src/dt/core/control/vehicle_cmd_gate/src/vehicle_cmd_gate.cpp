...
: Node("vehicle_cmd_gate", node_options), is_engaged_(false), updater_(this)
{
  ...
  twist_pub_ = this->create_publisher<geometry_msgs::msg::Twist>("output/cmd_vel", durable_qos);
  ...
  control_period_ = 1.0 / declare_parameter("control_rate", 20.0);
  ...
  pub_timer_ =
    create_wall_timer(second_type(control_period_), std::bind(&VehicleCmdGate::onPubControlTimer, this));
}

void VehicleCmdGate::onPubControlTimer() {
...
void VehicleCmdGate::publishControlCommands(const Commands & commands)
{
  ...
  cmd_vel.linear.x = filtered_commands.control.control.velocity;
  cmd_vel.angular.z = filtered_commands.control.control.steering_angle;  
  // Publish commands
