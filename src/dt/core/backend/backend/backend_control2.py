...
class BackendControl(Node):
    ...
    def __init__(self):
        ...
        cmd_test = ['rviz2']
        print('start')
        subprocess.Popen(cmd_test, shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print('end')
    ...
    def cp_node_callback(self, request, response):
        ...
        while(self.pro_cp_.poll()  is  None):{}
        cmd2 = ['du -sh /home/lukas/Desktop/bag']
        self.pro_cp_ = subprocess.Popen(cmd2,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print(self.pro_cp_.stdout.readlines())
        ...
        # file_ = open('/home/dzt/Desktop/1.txt', mode='wb+')
    ...
    def clear_node_callback(self, request, response):
        ...
        self.clear_state = 1
        print('Clear start!')
        cmd2 = ['rm -rf /home/dzt/Desktop/bag/*']
    ...
    def up_node_callback(self, request, response):
        ...
        cmd2 = ['sudo cp -r /mnt/usb/data/* /home/dzt/Desktop/data']
        self.pro_up = subprocess.Popen(cmd2,shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # file_ = open('/home/dzt/Desktop/err.txt', mode='wb+')
        # file_.write(self.pro_cp.stderr.readline())
        # file_ = open('/home/dzt/Desktop/out.txt', mode='wb+')
        # file_.write(self.pro_cp.stdout.readline())
        self.up_start = True
        return response
    def eject_node_callback(self, request, response):
        ...
        for i in EDISK:
            ...
            if ("sdb1" in str(i[:-1])):
                ...
                break
            if ("sdc1" in str(i[:-1])):
                cmd2 = ['sudo  eject /dev/sdc1']
                self.pro_eject = subprocess.Popen(cmd2,stdout = subprocess.PIPE,stderr = subprocess.PIPE,shell=True)
                break
            if ("sdd1" in str(i[:-1])):
                cmd2 = ['sudo  eject /dev/sdd1']
                self.pro_eject = subprocess.Popen(cmd2,stdout = subprocess.PIPE,stderr = subprocess.PIPE,shell=True)
                break
    ...
    def record_bag_callback(self, request, response):
        ...
        --qos-profile-overrides-path /home/dzt/.product/dtsimulation.ros2/record_qos.yaml
        """
