...
visualization_msgs::msg::MarkerArray visualization::roadLaneletsAsTriangleMarkerArray(
  const std::string ns, const lanelet::ConstLanelets & lanelets, 
  const std_msgs::msg::ColorRGBA c, const std_msgs::msg::ColorRGBA cl_road_avoid)
{
  visualization_msgs::msg::MarkerArray marker_array;
  visualization_msgs::msg::Marker marker;
  ...
  for (auto ll : lanelets) {
    ...
    if (ll.hasAttribute("avoid")) {
      auto value = ll.attribute("avoid").value();
      if (value == "true") {
        avoid = true;
      }
    }
    for (auto tri : triangles) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        marker.points.push_back(tri0[i]);
        if (avoid) {
          marker.colors.push_back(cl_road_avoid);
        } else {
          marker.colors.push_back(c);
        }
      }
