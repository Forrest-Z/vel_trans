...
void DummyPerceptionPublisherNode::timerCallback()
{
  ...
  std::string error;
  if (!tf_buffer_.canTransform("velodyne", /*src*/ "map", tf2::TimePointZero, &error)) {
    failed_tf_time = this->now();
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000, "map->velodyne is not available yet");
    return;
  }

  tf2::Transform tf_velodyne2map;
  try {
    geometry_msgs::msg::TransformStamped ros_velodyne2map;
    ros_velodyne2map = tf_buffer_.lookupTransform(
      /*target*/ "velodyne", /*src*/ "map", current_time, rclcpp::Duration::from_seconds(0.5));
    tf2::fromMsg(ros_velodyne2map.transform, tf_velodyne2map);
  ...
  for (size_t i = 0; i < objects_.size(); ++i) {
    ...
    createObjectPointcloud(
      ...
      tf_velodyne2map * tf_map2moved_object, pointcloud_ptr);
    ...
    tf2::Transform tf_velodyne2noised_moved_object;
    tf_velodyne2noised_moved_object =
      tf_velodyne2map * tf_map2moved_object * tf_moved_object2noised_moved_object;
    ...
    tf2::toMsg(tf_velodyne2noised_moved_object, feature_object.object.state.pose_covariance.pose);
    ...
    // check delete idx
    tf2::Transform tf_velodyne2moved_object;
    tf_velodyne2moved_object = tf_velodyne2map * tf_map2moved_object;
    double dist = std::sqrt(
      tf_velodyne2moved_object.getOrigin().x() * tf_velodyne2moved_object.getOrigin().x() +
      tf_velodyne2moved_object.getOrigin().y() * tf_velodyne2moved_object.getOrigin().y());
  ...
  if (enable_ray_tracing_) {
    ...
    ray_tracing_filter.setLeafSize(0.1, 0.1, 0.1);
    ...
    for (size_t i = 0; i < v_pointcloud.size(); ++i) {
      ...
      output_dynamic_object_msg.feature_objects.at(i).feature.cluster.header.frame_id = "velodyne";
  ...
  output_dynamic_object_msg.header.frame_id = "velodyne";
  output_dynamic_object_msg.header.stamp = current_time;
  output_pointcloud_msg.header.frame_id = "velodyne";
...
void DummyPerceptionPublisherNode::createObjectPointcloud(
  ...
  const double std_dev_y, const double std_dev_z, const tf2::Transform & tf_velodyne2moved_object,
  pcl::PointCloud<pcl::PointXYZ>::Ptr & pointcloud_ptr)
{
  std::normal_distribution<> x_random(0.0, std_dev_x);
  std::normal_distribution<> y_random(0.0, std_dev_y);
  std::normal_distribution<> z_random(0.0, std_dev_z);
  auto getBaseLinkTo2DPoint = [tf_velodyne2moved_object](double x, double y) -> pcl::PointXYZ {
    tf2::Transform tf_moved_object2point;
    tf2::Transform tf_velodyne2point;
    ...
    tf_velodyne2point = tf_velodyne2moved_object * tf_moved_object2point;
    pcl::PointXYZ point;
    point.x = tf_velodyne2point.getOrigin().x();
    point.y = tf_velodyne2point.getOrigin().y();
    point.z = tf_velodyne2point.getOrigin().z();
    return point;
  };
  ...
  const double horizontal_theta_step = (1 / 180.0) * M_PI;
  const double horizontal_min_theta = (-180.0 / 180.0) * M_PI;
  const double horizontal_max_theta = (180.0 / 180.0) * M_PI;

  const double min_z = -1.0 * (height / 2.0) + tf_velodyne2moved_object.getOrigin().z();
  const double max_z = 1.0 * (height / 2.0) + tf_velodyne2moved_object.getOrigin().z();
  ...
  const dummy_perception_publisher::msg::Object::ConstSharedPtr msg)
{
  switch (msg->action) {
    case dummy_perception_publisher::msg::Object::ADD: {
      ...
      // Use velodyne Z
      geometry_msgs::msg::TransformStamped ros_map2velodyne;
      try {
        ros_map2velodyne = tf_buffer_.lookupTransform(
          "map", "velodyne", rclcpp::Time(0), rclcpp::Duration::from_seconds(0.5));
        object.initial_state.pose_covariance.pose.position.z =
          ros_map2velodyne.transform.translation.z;
      } catch (tf2::TransformException & ex) {
