...
Node::Node(
    ...
    : node_options_(node_options), updater_(node)
{
  node_ = node;
  ...
  last_connect_time_publisher_ = node_->create_publisher<::std_msgs::msg::Int64>("lastconnecttime", 1);
  ...
  last_connect_time_timer_ = node_->create_wall_timer(
    std::chrono::milliseconds(1000),
    [this](){
      PublishLastConnectTime();
    });
  ...
  constrain_list_timer_ = node_->create_wall_timer(
    ...
    // diagnostic
    updater_.setHardwareID("cartographer");
    updater_.add("cartographer_diag", this, &Node::CheckCartographerStatus);
...
void Node::CheckCartographerStatus(
    diagnostic_updater::DiagnosticStatusWrapper& stat) {
  using diagnostic_msgs::msg::DiagnosticStatus;

  auto lastconnecttime = map_builder_bridge_->GetLastConnectTime();
  auto now = node_->now().nanoseconds() / 1000000000;
  int8_t level = DiagnosticStatus::OK;
  if (lastconnecttime - now < -50) {
    level = DiagnosticStatus::ERROR;
    stat.add("status", "Error");
  } else if (lastconnecttime - now < -30) {
  ...
  // add key-values
  stat.addf("now", "%d [s]", now);
  stat.addf("last_connect_time", "%d [s]", lastconnecttime);
  stat.addf("diff", "%d [s]", lastconnecttime - now);
...
void Node::PublishLastConnectTime(){
  absl::MutexLock lock(&mutex_);
  auto lastconnecttime = map_builder_bridge_->GetLastConnectTime();
  auto now = node_->now().nanoseconds() / 1000000000;
  std_msgs::msg::Int64 time;
  time.data = lastconnecttime-now;
  // LOG(WARNING) << lastconnecttime;
  last_connect_time_publisher_->publish(time);
}
