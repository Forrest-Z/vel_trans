  ...
  const std::unique_ptr<Trajectories> & prev_trajs)
{
  ...
  for (const auto & object : objects) {
    ...
    if (isAvoidingObject(
          polygon_points, object, clearance_map, map_info, path_points_inside_area, traj_param, prev_trajs)) {
...
bool isAvoidingObject(
  ...
  const std::vector<autoware_planning_msgs::msg::PathPoint> & path_points,
  const TrajectoryParam & traj_param, const std::unique_ptr<Trajectories> & prev_trajs)
{
  if (path_points.empty()) {
    return false;
  }
  // if (isObjectNearBend(object, path_points)) {
  //   return false;
  // }
  ...
  if (isObjectNearCentre(nearest_path_point.pose, object, clearance_map, map_info)) {
    return false;
  }
  ...
  // if (nearest_idx > path_points.size() * 0.5) {
  //   return false;
  // }
  ...
  //   map_info.resolution;
  const double object_dist = std::sqrt(rel_p.x * rel_p.x + rel_p.y * rel_p.y);
  if (
  ...
  // TODO(jiheng): maybe consider put below values to param 
  if (object_dist < traj_param.center_line_width * 0.7) {
    return true;
  }
  if (prev_trajs == nullptr) {
    return false;
  }
  const std::vector<autoware_planning_msgs::msg::TrajectoryPoint>  pre_optimized_points = prev_trajs->model_predictive_trajectory; 
  auto interpolated_points = util::getInterpolatedPoints(polygon_points.points_in_map,
    traj_param.center_line_width * 0.3);
  const int nearest_idx_pre =
    util::getNearestIdx(pre_optimized_points, object.state.pose_covariance.pose.position);
  const auto nearest_pre_optimized_points = pre_optimized_points[nearest_idx_pre];
  const auto dis_p_t = util::transformToRelativeCoordinate2D(
    nearest_pre_optimized_points.pose.position, nearest_path_point.pose);
  const double p_t_dist = std::sqrt(dis_p_t.x * dis_p_t.x + dis_p_t.y * dis_p_t.y);
  for (const auto & point : interpolated_points) {
    double min_dist = getNearestDistance(path_points, point);
    if (min_dist < traj_param.center_line_width * 0.8) {
      return true;
    }
    double min_dist_pre = getNearestDistance(pre_optimized_points, point);
    if (p_t_dist > object_dist) {
      if (min_dist_pre < traj_param.center_line_width * 2) {
      return true;
      }
    } else {
      if (min_dist_pre < traj_param.center_line_width * 0.8) {
      return true;
      }
    }
  ...
  for (const auto & point : polygon_points.points_in_map) {
    double min_dist = getNearestDistance(path_points, point);
    if (min_dist < traj_param.center_line_width * 0.8) {
      return true;
    }
  ...
  const std::vector<autoware_planning_msgs::msg::PathPoint> & path_points)
{
  ...
  int closest_idx =
    util::getNearestIdx(path_points, object.state.pose_covariance.pose.position);
  int max_idx = closest_idx + inter_steps > static_cast<int>(path_points.size()) -2 
    ? static_cast<int>(path_points.size()) -1 : closest_idx + inter_steps;
  int min_idx = closest_idx;

  double bend_angle = 0;
  const double max_bend_angle = 60;
  for (int i = min_idx; i < max_idx; i += 1) {
    const double back_x = path_points[i].pose.position.x - path_points[i - 1].pose.position.x;
    const double back_y = path_points[i].pose.position.y - path_points[i - 1].pose.position.y;
    const double forward_x = path_points[i + 1].pose.position.x - path_points[i].pose.position.x;
    const double forward_y = path_points[i + 1].pose.position.y - path_points[i].pose.position.y;
    double A = back_x * forward_x + back_y * forward_y;
    double B = std::sqrt(back_x * back_x + back_y * back_y) * 
    std::sqrt(forward_x * forward_x + forward_y * forward_y);
    const double angle = std::acos(A / B);
    bend_angle += std::fabs(angle);
  }
  if (bend_angle > max_bend_angle * M_PI/ 180) {
    RCLCPP_INFO(
      rclcpp::get_logger("isObjectNearBend"), "[Avoidance]The Object near the bend");
    return true;
  }
  return false;
}

bool isObjectNeedExtend(
...
{
  if (isObjectNearCentre(nearest_path_point_pose, object, clearance_map, map_info)) {
    return false;
  }
  return true;
}

bool isObjectNearCentre(
  const geometry_msgs::msg::Pose & nearest_path_point_pose,
  const autoware_perception_msgs::msg::DynamicObject & object, const cv::Mat & clearance_map,
  const nav_msgs::msg::MapMetaData & map_info)
{
  ...
  //TODO(qianfeng): build a function getClearance()
  constexpr double ray_origin_dist_scale = 1.0;
  const auto obj_point_image =
    util::transformMapToImage(object.state.pose_covariance.pose.position, map_info);
  const float obj_clearance = clearance_map.ptr<float>(static_cast<int>(
                            obj_point_image.y))[static_cast<int>(obj_point_image.x)] *
                          map_info.resolution * ray_origin_dist_scale;
  //calculate the max clearance
  double ds = map_info.resolution;
  double lane_width = 7.5;
  int n = static_cast<int>(lane_width / ds);
  double traversed_dist = 0;
  auto traver_vec = obj2origin.normalized();
  float max_clearance = 0;
  for (int i = 0; i < n; ++i) {
    traversed_dist += ds;
    geometry_msgs::msg::Point new_position;
    new_position.x = nearest_path_point_pose.position.x + traversed_dist * traver_vec[0] ;
    new_position.y = nearest_path_point_pose.position.y + traversed_dist * traver_vec[1] ;
    auto new_position_image = util::transformMapToImage(new_position, map_info);
    float new_clearance = clearance_map.ptr<float>(static_cast<int>(
                            new_position_image.y))[static_cast<int>(new_position_image.x)] *
                          map_info.resolution * ray_origin_dist_scale;
  ...
  if (obj_clearance + 0.3 > max_clearance) {
    return true;
  }
  return false;
}
...
std::vector<cv::Point> getExtendedCVPolygon(
  ...
  const nav_msgs::msg::MapMetaData & map_info)
{
  boost::optional<Edges> optional_edges;
  if (isObjectNeedExtend(nearest_path_point_pose, object,  clearance_map, map_info)) {
    optional_edges = getExtendEdges(
      points_in_image, points_in_map, nearest_path_point_pose, object, clearance_map, map_info);
  } else {
    optional_edges = getEdges(
      points_in_image, points_in_map, nearest_path_point_pose, object, clearance_map, map_info);
  }
  ...
  const nav_msgs::msg::MapMetaData & map_info)
{
  ...
  double* minp = &minv;
  double* maxp = &maxv;
  cv::minMaxIdx(clearance_map,minp,maxp);
...
boost::optional<Edges> getExtendEdges(
  const std::vector<geometry_msgs::msg::Point> & points_in_image,
  const std::vector<geometry_msgs::msg::Point> & points_in_map,
  const geometry_msgs::msg::Pose & nearest_path_point_pose,
  const autoware_perception_msgs::msg::DynamicObject & object, const cv::Mat & clearance_map,
  const nav_msgs::msg::MapMetaData & map_info)
{
  ...
  double ds = map_info.resolution;
  double min_width = 2;
  geometry_msgs::msg::Point gradient_position;
  const Eigen::Vector2d gradient_vec = obj2origin.normalized() * ds;
  gradient_position.x = object.state.pose_covariance.pose.position.x + gradient_vec[0];
  gradient_position.y = object.state.pose_covariance.pose.position.y + gradient_vec[1];
  auto gradient_point_image =
     util::transformMapToImage(gradient_position, map_info);
  double gradient_clearance = clearance_map.ptr<float>(static_cast<int>(
                            gradient_point_image.y))[static_cast<int>(gradient_point_image.x)] *
                          map_info.resolution * ray_origin_dist_scale;
  Eigen::Vector2d obj2ray_origin;
  float extend_scale = 0.0;
  if ( gradient_clearance >= obj_clearance) {
      obj2ray_origin = 2 * obj2origin.normalized() * (obj_clearance + min_width);
      extend_scale = obj2ray_origin.norm() + 1.2 * obj_clearance;
  } else {
      obj2ray_origin = -2 * obj2origin.normalized() * (obj_clearance + min_width);
      extend_scale = obj2ray_origin.norm() + 1.2 * obj_clearance;
  }
  ...
  const Eigen::Vector2d point2back_edge(
    ...
    points_in_image[edges.back_idx].y - ray_origin_image.y);
  const Eigen::Vector2d point2extended_front_edge =
    point2front_edge.normalized() * ( extend_scale / max_sin) * (1 / map_info.resolution);
  const Eigen::Vector2d point2extended_back_edge =
    point2back_edge.normalized() * ( extend_scale / min_sin) * (1 / map_info.resolution);
...
{
  const int nearest_idx = util::getNearestIdx(path_points,
      point, 0);
  const auto nearest_path_point = path_points[nearest_idx];
  const auto rel_p = util::transformToRelativeCoordinate2D(
    point, nearest_path_point.pose);
  return std::sqrt(rel_p.x * rel_p.x + rel_p.y * rel_p.y);
}
float getNearestDistance(
  const std::vector<autoware_planning_msgs::msg::TrajectoryPoint> & optimized_points,
  const geometry_msgs::msg::Point & point)
{
  const int nearest_idx = util::getNearestIdx(optimized_points,
      point, 0);
  const auto nearest_optimized_points = optimized_points[nearest_idx];
  const auto rel_p = util::transformToRelativeCoordinate2D(
    point, nearest_optimized_points.pose);
  return std::sqrt(rel_p.x * rel_p.x + rel_p.y * rel_p.y);
}
  ...
  const std::unique_ptr<Trajectories> & prev_trajs)
{
  ...
  cv::Mat objects_image = drawObstaclesOnImage(
    ...
    traj_param, &debug_avoiding_objects, prev_trajs);
