  ...
  const std::vector<autoware_perception_msgs::msg::DynamicObject> & objects, DebugData * debug_data)
{
  ...
  CVMaps cv_maps = process_cv::getMaps(enable_avoidance, path, objects, traj_param_, debug_data, prev_trajs);
  ...
  // if ( extended_traj_points.size() == 0) {
  //     RCLCPP_WARN(rclcpp::get_logger("EBPathOptimizer"),
  //     "[Avoidance] Extend trajectory is none");
  // }
  ...
  DebugData * debug_data)
{
  if (static_cast<int>(optimized_points.size()) <= traj_param_.num_fix_points_for_extending) {
    ...
    RCLCPP_WARN(rclcpp::get_logger("EBPathOptimizer"),
      "[Avoidance] optimized_points %ld,  %d", optimized_points.size(), traj_param_.num_fix_points_for_extending);
  ...
  if (
    ...
    RCLCPP_INFO(rclcpp::get_logger("EBPathOptimizer"), "[Avoidance] Not extend trajectory");
    RCLCPP_WARN(rclcpp::get_logger("EBPathOptimizer"),
      "[Avoidance] accum_arc_length %f,  %f", accum_arc_length, traj_param_.trajectory_length);
    return std::vector<autoware_planning_msgs::msg::TrajectoryPoint>{};
  }
...
std::vector<double> EBPathOptimizer::solveQP(const OptMode & opt_mode)
{
  ...
  if (opt_mode == OptMode::Normal) {
    ...
    if (solution_status != 1) {
      RCLCPP_INFO(
       rclcpp::get_logger("EB_PATHOptimizer"), "[Avoidance] Normal Solution Error");
      return {};
    }
    if (!isOsqpResultGood(optimized_points, normal_a_matrix_, normal_lb, normal_ub)) {
      return {};
    }
    ...
    if (solution_status != 1) {
      RCLCPP_INFO(
       rclcpp::get_logger("EB_PATHOptimizer"), "[Avoidance] Extending Solution Error");
      return {};
    }
    if (!isOsqpResultGood(optimized_points, extend_a_matrix_, extend_lb, extend_ub)) {
      return {};
    }
    ...
    if (solution_status != 1) {
      RCLCPP_INFO(
       rclcpp::get_logger("EB_PATHOptimizer"), "[Avoidance] Visualizing Solution Error");
      return {};
    }
  }
...
bool EBPathOptimizer::isOsqpResultGood(std::vector<double> optimized_points, Eigen::MatrixXd boud_matrix,
  ...
  for (int i = 0; i < traj_param_.num_sampling_points; ++i) {
    if (std::isnan(optimized_points[i]) || std::isnan(optimized_points[i + traj_param_.num_sampling_points]) ) {
      RCLCPP_INFO(
        rclcpp::get_logger("EB_PATHOptimizer"), "[Error] Optimized Point is nan");
      return false;
    }
    le = optimized_points[i] * boud_matrix(i, i) +
      optimized_points[i + traj_param_.num_sampling_points] *
      boud_matrix(i, i + traj_param_.num_sampling_points) - lb[i] ;
    ue = optimized_points[i] * boud_matrix(i, i) +
      optimized_points[i + traj_param_.num_sampling_points] *
      boud_matrix(i, i + traj_param_.num_sampling_points) - ub[i] ;
    // TODO(qianfeng):there should be a more reasonable tolerance, associated with boundary
    if ( le < -1 * tol || ue > tol ) {
      RCLCPP_INFO(
        rclcpp::get_logger("EB_PATHOptimizer"), "The result is not satisfied with the bound , so return none ");
      RCLCPP_INFO(
        rclcpp::get_logger("EB_PATHOptimizer"), "The i=%d, lb=%f , the ub=%f, le=%f, ue=%f", i, lb[i], ub[i], le, ue);
      return false;
    }
  ...
  const std::vector<ConstrainRectangle> & rectangle_points, const OptMode & opt_mode)
{
  ...
  if (opt_mode == OptMode::Normal) {
    normal_a_matrix_ = A;
    normal_lb = lower_bound;
    normal_ub = upper_bound;
    osqp_solver_ptr_->updateBounds(lower_bound, upper_bound);
    osqp_solver_ptr_->updateA(A);
  } else if (opt_mode == OptMode::Extending) {
    extend_a_matrix_ = A;
    extend_lb = lower_bound;
    extend_ub = upper_bound;
  ...
  const geometry_msgs::msg::Point & opposite_point)
{
  ...
  // TODO(qianfeng): The judgment here is to prevent the sparsity property of matrix A from changing, 
  //but this is not the best way. Because the form of A in MPT is more complex, 
  //fundamental changes need to be made from osqp::OSQPinterface::updateA();
  if (std::fabs(slope) < epsilon_) {
    constrain_point.x_coef = -1 * epsilon_;
  } else {
