  ...
  const geometry_msgs::msg::Pose & ego_pose, DebugData * debug_data)
{
  ...
  if (!isResultMatchBound(ref_points, optimized_control_variables.get(), mpt_matrix.get(), debug_data)) {
    RCLCPP_INFO(
        rclcpp::get_logger("MPTOptimizer"), "Warning: MPT result not match bound, return none");
    return boost::none;
  }
  const auto mpt_points =
    getMPTPoints(ref_points, optimized_control_variables.get(), mpt_matrix.get(), smoothed_points);
  if (!isCurvatureGood(mpt_points)) {
    RCLCPP_INFO(
        rclcpp::get_logger("MPTOptimizer"), "Warning: Curvature too large, return none");
    return boost::none;
  }
  ...
  DebugData * debug_data) const
{
  ...
  const auto first_it = ref_points.begin();
  int num_points = std::min(
    static_cast<int>(ref_points.size()) - 1, traj_param_ptr_->num_sampling_points);
  ...
  DebugData * debug_data)
{
  ...
  if (const_m.linear.size() == 0) {
    RCLCPP_INFO(
      rclcpp::get_logger("MPTOptimizer"),  "Warning: No constraint");
    return boost::none;
  }
  osqp_solver_ptr_ = std::make_unique<osqp::OSQPInterface>(
    ...
    1.0e-5);
  osqp_solver_ptr_->updateEpsRel(1.0e-5);
  ...
  //TODU(qianfeng): max_curve should be a mpt para
  double max_curve = 0.8;
  for (int i = 0; i < static_cast<int>(ref_points.size()); i++){
    if (std::fabs(result_vec[i]) > max_curve) {
      RCLCPP_INFO(rclcpp::get_logger("MPTOptimizer"), 
      "[Warning] Curve too large: optimized point at %d = %f", i, result_vec[i]);
      return boost::none;
    }
  }
...
bool MPTOptimizer::isResultMatchBound(
  std::vector<ReferencePoint> & ref_points, const Eigen::VectorXd & Uex,
  const MPTMatrix & mpt_matrix, DebugData * debug_data) const
{
  const int DIM_X = vehicle_model_ptr_->getDimX();
  const auto base_bound = debug_data->bounds;
  const auto x0 = ref_points.front().optimized_state;
  Eigen::VectorXd Xex = mpt_matrix.Aex * x0 + mpt_matrix.Bex * Uex + mpt_matrix.Wex;

  {
    const double lat_error = x0(0);
    if (lat_error < base_bound[0].c0.lb || lat_error > base_bound[0].c0.ub) {
      RCLCPP_INFO(
        rclcpp::get_logger("MPTOptimizer"),  
        "Result not match bound: bound(0)=%f ~ %f, but result(0) = %f", 
        base_bound[0].c0.lb, base_bound[0].c0.ub, lat_error);
      return false;
    }
  }
  ...
  for (std::size_t i = 1; i < ref_points.size(); ++i) {
    const double lat_error = Xex((i - 1) * DIM_X);
    if (lat_error < base_bound[i].c0.lb || lat_error > base_bound[i].c0.ub) {
      RCLCPP_INFO(
        rclcpp::get_logger("MPTOptimizer"),  
        "Result not match bound: bound(%d)=%f ~ %f, but result(%d) = %f", static_cast<int>(i),
        base_bound[i].c0.lb, base_bound[i].c0.ub, static_cast<int>(i), lat_error);
      return false;
    }
  }
  return true;
...
bool MPTOptimizer::isCurvatureGood(
  const std::vector<autoware_planning_msgs::msg::TrajectoryPoint> & traj_points) const
{
  int num_points = static_cast<int>(traj_points.size());

  /* calculate curvature by circle fitting from three points */
  geometry_msgs::msg::Point p1, p2, p3;
  const double max_curve = 1.0;
  for (int i = 1; i < num_points - 1; ++i) {
    p1 = traj_points[i - 1].pose.position;
    p2 = traj_points[i].pose.position;
    p3 = traj_points[i + 1].pose.position;
    double den = std::max(
      util::calculate2DDistance(p1, p2) * util::calculate2DDistance(p2, p3) *
        util::calculate2DDistance(p3, p1),
      0.0001);
  ...
  DebugData * debug_data) const
{
  ...
  for (const auto & point : ref_points) {
    ...
    if (!lat_bounds_0 || !lat_bounds_1 || !lat_bounds_2) {
      RCLCPP_INFO(
        rclcpp::get_logger("MPTOptimizer"),  "Waning: bound error, return none");
      return {};
  ...
  const bool enable_avoidance, const ReferencePoint & ref_point, const CVMaps & maps) const
{
  ...
  while (true) {
    ...
    if (bound_candidate.get()[0] > rough_road_bound.get()[0] || bound_candidate.get()[1] < rough_road_bound.get()[1]) {
      break;
    }
  ...
  const auto roadside_bound = getRoadsideBound(ref_point, maps, 
  mpt_param_ptr_->clearance_from_road, broad_bound);
  broad_bound = roadside_bound.get();
  if (broad_bound.empty()) {
    return boost::none;
  } else {
    return broad_bound;
...
boost::optional<std::vector<double>> MPTOptimizer::getRoadsideBound(
  const ReferencePoint & ref_point, const CVMaps & maps,
  const double min_road_clearance, std::vector<double> rough_road_bound) const
{
  geometry_msgs::msg::Point new_position;
  new_position.x = ref_point.p.x;
  new_position.y = ref_point.p.y;
  auto original_clearance = getClearance(maps.clearance_map, new_position, maps.map_info);
  auto left_bound = rough_road_bound[0];
  auto right_bound = rough_road_bound[1];
  if (original_clearance < min_road_clearance + 0.55 && rough_road_bound[0] > 0.1
     && rough_road_bound[1] < -0.1) {
    if (std::fabs(rough_road_bound[0]) < std::fabs(rough_road_bound[1])) {
      left_bound = 0.01;
    } else {
      right_bound = -0.01;
    }
  ...
  DebugData * debug_data) const
{
  ...
  if (bounds.empty()) {
    RCLCPP_INFO(
        rclcpp::get_logger("MPTOptimizer"),  "warning: No bounds");
    return {};
  }
  ...
  //Steer limit constraint
  Eigen::MatrixXd A_s = Eigen::MatrixXd::Zero(3 * N_ref * N_point + N_ref + N_ref, N_dec);
  Eigen::VectorXd lb_s = Eigen::VectorXd::Constant(3 * N_ref * N_point + N_ref + N_ref, -osqp::INF);
  Eigen::VectorXd ub_s = Eigen::VectorXd::Constant(3 * N_ref * N_point + N_ref + N_ref, osqp::INF);
  {
    double degree_limit = 0.79;

    A_s.block(0, 0, 3 * N_ref * N_point + N_ref, N_dec) = A;
    lb_s.segment(0, 3 * N_ref * N_point + N_ref) = lb;
    ub_s.segment(0, 3 * N_ref * N_point + N_ref) = ub;
    // S := [I | O | O]
    Eigen::MatrixXd C = Eigen::MatrixXd::Identity(N_ref, N_ref);

    // Assign
    A_s.block(3 * N_point * N_ref + N_ref, 0, N_ref, N_ref) = C;
    for (size_t i = 0; i < N_ref; ++i) {
      lb_s(3 * N_point * N_ref + N_ref + i) = -degree_limit;
      ub_s(3 * N_point * N_ref + N_ref + i) = degree_limit;
    }
  }

  ConstraintMatrix constraint_matrix;

  constraint_matrix.linear = A_s;

  for (int i = 0; i < lb_s.size(); ++i) {
    constraint_matrix.lower_bound.push_back(lb_s(i));
    constraint_matrix.upper_bound.push_back(ub_s(i));
  }
  ...
  const double min_obj_clearance, const bool search_expanding_side) const
{
  ...
  // TODO(qianfeng):the lane_width should be a param
  constexpr double lane_width = 7.5;
  int n = static_cast<int>(lane_width / ds);
  //TODO(banzhang): hard fix
  geometry_msgs::msg::Point origin;
  origin.x = ref_point.p.x;
  origin.y = ref_point.p.y;
  double clearance_last = getClearance(maps.clearance_map, origin, maps.map_info).get();
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (search_expanding_side) {
      // TODO(qianfeng): maybe set 0.2 as config params or use better algorithm
      if (clearance.get() > min_road_clearance && obj_clearance.get() > min_obj_clearance + 0.2) {
        break;
      }
    } else {
      //TODO(banzhang): hard fix
      if (clearance.get() < mpt_param_ptr_->clearance_from_road && clearance.get() < clearance_last) {
        break;
      }

      clearance_last = clearance.get();
  ...
  const std::vector<double> & rough_road_bound, const bool search_expanding_side) const
{
  ...
  // Select better contain space when left and right blocking area is far from path.
  int n = static_cast<int>(lane_width / ds);
  double rel_value = 0;
  if (rough_road_bound[1] < 0 && rough_road_bound[0] > lane_width - 1 && search_expanding_side && initial_value > 0) {
    rel_value = (rough_road_bound[0] / (rough_road_bound[0] - rough_road_bound[1])) * initial_value;
  } else {
    rel_value = initial_value;
  }
